...
├── requirements.txt                 # MUST stay in the same folder as app.py
├── runtime.txt                      # Pins Python version for the host
├── DEPLOYMENT.md                    # Full deploy checklist & troubleshooting
├── LICENSE
├── .gitignore
└── README.md
```

> ⚠️ **Deploying from a monorepo / nested folder?** `requirements.txt` and
> `runtime.txt` must live in the exact same directory as `app.py` — see
> [DEPLOYMENT.md](DEPLOYMENT.md) for the full checklist, including the fix for
> the common `ModuleNotFoundError` on first deploy.
...
3. Set the main file path to `app.py` (or `<subfolder>/app.py` if this
   project lives inside a larger repo — see the warning above).
4. Deploy — the app reads pre-trained artifacts from `/model`, so no retraining is needed at deploy time.
5. If the build fails, see [DEPLOYMENT.md](DEPLOYMENT.md) for the full
   troubleshooting checklist (covers the common `ModuleNotFoundError`).
